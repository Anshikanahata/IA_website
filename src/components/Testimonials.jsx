    ...
    <section ref={sectionRef} className="py-20 bg-gradient-to-br from-gray-50 to-indigo-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`transition-all duration-1000 ${
          ...
          <h2 className="text-4xl font-light text-center mb-4 text-gray-900 tracking-tight">What Advisors Say</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            <div
              key={index}
              className={`bg-white rounded-xl p-8 shadow-layered hover:shadow-layered-lg transition-all duration-300 transform hover:scale-[1.02] border border-gray-100 ${
                isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'
              }`}
              style={{ transitionDelay: `${index * 150}ms` }}
            >
              <div className="text-saffron-500 text-5xl mb-4 font-serif leading-none">"</div>
              <p className="text-gray-700 mb-6 leading-relaxed italic text-base">
                {testimonial.quote}
              </p>
              <div className="border-t border-gray-200 pt-4">
                <div className="font-medium text-gray-900 mb-1">{testimonial.author}</div>
